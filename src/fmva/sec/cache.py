...
class TextCacheEntry:
    """Cached text payload and metadata."""
...
class TextDiskCache:
    """URL-keyed UTF-8 text cache for filing documents and XBRL instances."""
    ...
    def _path(self, url: str) -> Path:
        digest = hashlib.sha256(url.encode("utf-8")).hexdigest()
        return self.directory / f"{digest}.text.json"

    def get(self, url: str) -> TextCacheEntry | None:
        """Return fresh cached text, or None for missing, stale, or malformed data."""
        ...
        try:
            ...
            if not isinstance(payload, str):
                return None
            return TextCacheEntry(payload=payload, fetched_at=fetched_at, url=str(raw["url"]))
        except (OSError, KeyError, TypeError, ValueError, json.JSONDecodeError):
            return None

    def put(self, url: str, payload: str) -> None:
        """Atomically write one UTF-8 text response to cache."""
