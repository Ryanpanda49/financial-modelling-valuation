...
FIXTURE = Path("data/sample/aapl_fy2021_2025_history.json")


def test_public_aapl_fixture_runs_complete_offline_model() -> None:
    ...
    model = ValuationModel.from_history_json(
        FIXTURE,
        forecast_assumptions_path="examples/aapl/forecast_assumptions.yaml",
        valuation_assumptions_path="examples/aapl/valuation_assumptions.yaml",
    )
    result = model.run()
    ...
    assert result.dcf.implied_share_price == pytest.approx(112.05, abs=0.02)
